                ...
                <div className="card-container">
                  {roofConditions.map((condition) => (
                    <Card
                      ...
                      img={condition.icon}
                      onClick={() =>
                        handleUserAnswer({ roof_condition: condition.id })
                      }
                      isOneBg={true}
                      containerPadding={"20px 20px 0px 20px"}
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        handleUserAnswer({
                          ...
                          is_mannual_provider:
                            e.target.value.length > 0 ? true : null,
                          provider: null,
                        });
                      }}
                    />
          ...
          case 6:
            return (
              <>
                ...
                <div
                  className="card-container-emojie"
                ...
                >
                  {utilityBillsList.map((bill, idx) => (
                    <Card
                      ...
                      onClick={() =>
                        handleUserAnswer({ utility_bill_amount: bill.id })
                      }
                      imgHeight={64}
                      imgWidth={64}
                      isActive={quizData.utility_bill_amount === bill.id}
                      isDisabled={quizData.hasOwnProperty(
                        "utility_bill_amount"
                      )}
                ...
                >
                  {creditScoreList.map((score) => (
                    <Card
                      ...
                      onClick={() =>
                        handleUserAnswer({ credit_score: score.id })
                      }
                      imgHeight={64}
                      imgWidth={64}
